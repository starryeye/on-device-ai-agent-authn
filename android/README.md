...
위 확인은 전부 영어 프롬프트로 진행했다 — `adb shell input text`는 한글(Hangul)을 입력기에 주입하지 못해서, 이 문서가 안내하는 "배터리 몇 퍼센트야?"를 그대로 자동으로 재현할 수 없었기 때문이다. 대신 클립보드 경유(Chrome 검색창에 URL 인코딩으로 한글을 렌더링한 뒤 복사 → 앱 입력창에 붙여넣기)로 실제 한글 프롬프트를 넣어 같은 대화에서 이어서 확인해 봤다:

4. 외부에서 배터리를 다시 변경: `adb shell dumpsys battery set level 77`

5. "배터리 몇 퍼센트야" (클립보드 붙여넣기로 입력, 실제 한글)
   → `툴 호출: get_battery_level` 줄이 뜨지 않음 — 툴이 호출되지 않았다.
   → "배터리는 42%입니다." (실제 값 77%가 아니라 방금 전 영어 대화의 42%를 그대로 반복)

즉 이번 실행에서는 한글 프롬프트가 툴 호출을 이끌어내지 못했고, 모델은 도구를 다시 부르는 대신 직전 답변의 숫자를 재사용했다. 영어 지시문으로 학습/구성된 2B 모델이 한글 트리거에서 안정적이지 않을 수 있다는 우려(아래 "알려진 한계" 참고)가 실제로 재현된 사례다. 앱의 준비 메시지와 이 문서의 안내는 한글 "배터리 몇 퍼센트야?"를 계속 쓰지만, 이 경로는 영어 경로와 달리 툴 호출이 보장되지 않는다는 점을 알아둘 것.
